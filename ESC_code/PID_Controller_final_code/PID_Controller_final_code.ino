...
void loop(){
  ...
  Serial.print("pitch : ");Serial.println(Pitch);
  Serial.print("Roll : ");Serial.println(Roll);
  PID_pitch=PID(pitcherror,&pitchintegralgain,&previouspitcherror);             //putting the errors throught the PID for pitch to get the value to feed the servo
  PID_roll=PID(rollerror,&rollintegralgain,&previousrollerror);                 //putting the errors throught the PID for roll to get the value to feed the servo
  servo_write(PID_pitch_1(PID_pitch),PID_roll_2(PID_roll));                     //giving the servos the values to write 
   Serial.print("this is pitch angle");Serial.println(PID_pitch_1(PID_pitch));   //testing if the values given are reasonnable
  //Serial.print("this is roll angle");Serial.println(PID_roll_2(PID_roll));      //testing if the values that are given are reasonnable
  //change the value of PID_pitch_X or PID_roll_X to 2 or 1 depending on the case needed, 2 and 1 cover all possible scenarion
  //Serial.print("this is pitch angle");Serial.println(Yaw);   //testing if the values given are reasonnable
  //Serial.print("this is roll angle");Serial.println(Pitch);      //testing if the values that are given are reasonnable
  }
